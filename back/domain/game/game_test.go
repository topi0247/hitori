...
import (
	...
	"github.com/topi0247/hitori/domain/game"
)
...
func TestJudge_ExactMatch(t *testing.T) {
	...
	result, err := game.Judge(cards, answers)
	if err != nil {
	...
	for _, r := range result.Cards {
		...
		if r.MatchPoints != game.PointsExact {
			t.Errorf("UUID=%s: MatchPoints=%d, want %d", r.UUID, r.MatchPoints, game.PointsExact)
		}
	}
...
func TestJudge_AdjacentMatch(t *testing.T) {
	...
	for _, r := range result.Cards {
		...
		case "a", "b":
			...
			if r.MatchPoints != game.PointsAdjacent {
				t.Errorf("UUID=%s: MatchPoints=%d, want %d", r.UUID, r.MatchPoints, game.PointsAdjacent)
			}
		case "c":
			if !r.IsCorrect {
...
func TestJudge_InvalidAnswer(t *testing.T) {
	...
	_, err := game.Judge(cards, answers)
	if err == nil {
